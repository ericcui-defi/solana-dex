...
fn test_remove_liquidity() {
    ...
    let remove_liquidity_instruction = Instruction::new_with_bytes(
        program_id,
        &dex::instruction::RemoveLiquidity { lp_amount: 10 }.data(),
        dex::accounts::RemoveLiquidity {
            user: payer.pubkey(),
            pool: pool,
            token_vault_a: token_vault_a,
            token_vault_b: token_vault_b,
    ...
    // Running liquidity remove
    let blockhash = svm.latest_blockhash();
    let msg = Message::new_with_blockhash(&[remove_liquidity_instruction], Some(&payer.pubkey()), &blockhash);
    let tx = VersionedTransaction::try_new(VersionedMessage::Legacy(msg), &[&payer]).unwrap();
    let res = svm.send_transaction(tx);

    // Assertions
    if let Err(e) = &res {
        panic!("remove_liquidity failed: {:?}\nlogs:\n{}", e.err, e.meta.logs.join("\n"));
    }
